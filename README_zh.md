...
00:00 → ping → 窗口 00:00–05:00
         ↑ 00:00+5h=05:00，但 05:00 的窗口会在 10:00 结束，10:00 > 次日 09:00
         → 跳过 05:00，直接等到次日 09:00 重新锚定
```

每天最多 4 次 ping。脚本只在"这次 ping 开启的窗口能在下一个 daily-reset 前完整结束"时才续期，避免新窗口覆盖次日锚点。

**实际发送时间**略晚于目标时间，采用递增 buffer 抵消时钟精度误差：第 1 次 +10s、第 2 次 +20s、第 3 次 +30s、第 4 次 +40s，次日重置为 +10s。
